/********************************************************************
	Framework Script
	Class: 	GreyFramework::BetterList
	Author:	
	Created:	
	Note:	
*********************************************************************/
...
    public void Sort (System.Comparison<T> comparer) { mList.Sort(comparer); }
        ...
        {
            if(buffer != null&&buffer.Length>cnt)
                return;
            T[] newList =  new T[cnt];
        ...
        {
            if (safeReleaseSize > 0 && buffer != null && buffer.Length >= safeReleaseSize)
            {
                System.Array.Clear(buffer, buffer.Length - safeReleaseSize, safeReleaseSize);
            }
        ...
        public void AddRange(BetterList<T> list, int length)
        {
        ...
        public bool Contains(T item)
        {
            ...
            for (int i = 0; i < size; ++i) if (comparer.Equals(buffer[i],item)) return true;
            return false;
        }
        ...
        public int IndexOf(T item)
        {
            ...
            for (int i = 0; i < size; ++i) if (comparer.Equals(buffer[i],item)) return i;
            return -1;
        }
